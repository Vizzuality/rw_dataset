...
module WidgetService
  class << self
    ...
    def get_widget(options)
      ...
      body = {}
      body['widget'] = {}
      body['widget']['ids'] = options['ids'] if options['ids'].present?
      body['widget']['app'] = options['app'] if options['ids'].present? && options['app'].present?

      method = options['ids'].present? ? 'post' : 'get'

      if options['ids'].present?
        url  = "#{Service::SERVICE_URL}/widget/find-by-ids"
      else
        url  = "#{Service::SERVICE_URL}/dataset"
        url += "/#{options['dataset_id']}/widget"
